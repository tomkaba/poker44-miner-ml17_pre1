...
def iter_benchmark_chunks(conn: sqlite3.Connection, limit: Optional[int] = None) -> Iterator[Tuple[str, int, str, List[dict]]]:
    query = (
        "SELECT t.chunk_hash, COALESCE(t.truth_value, 0), COALESCE(t.truth_label, ''), d.chunk_raw "
    ...
    if limit is not None:
        query += " LIMIT ?"
        params = (limit,)
    for chunk_hash, truth_value, truth_label, chunk_raw in conn.execute(query, params):
        try:
        ...
        if not isinstance(chunk, list) or not chunk:
            continue
        yield str(chunk_hash), int(truth_value), str(truth_label), chunk
...
def export_dataset(db_path: Path, output_dir: Path, limit: Optional[int]) -> Dict[str, object]:
    ...
    try:
        with hand_output.open("w", encoding="utf-8") as hand_handle, chunk_output.open("w", encoding="utf-8") as chunk_handle:
            for chunk_hash, truth_value, truth_label, chunk in iter_benchmark_chunks(conn, limit=limit):
